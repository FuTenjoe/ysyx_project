...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu,   I, R(dest) = src1 < ( src2) ? 1:0); //buqueding
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq  ,   B, s->dnpc = (src1==src2) ? s->pc + dest :  s->pc+4  );
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne  ,   B, s->dnpc = (src1==src2) ? s->pc+4 :  s->pc + dest  );
  ...
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai ,   I, R(dest) = SEXT(BITS((SEXT(src1,32) >> src2), 31, 0),32) );
  ...
  INSTPAT("??????? ????? ????? 011 ????? 01100 11", sltu,    R, R(dest) = (unsigned int)src1 < (unsigned int) src2 ? 1:0);
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge  ,   B, s->dnpc = (SEXT(src1,32) >= SEXT(src2,32)) ? s->pc + dest :  s->pc+4  );
...
//if-else
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt  ,   B, s->dnpc = (SEXT(src1,32)< SEXT(src2,32)) ? s->pc + dest :  s->pc+4 );
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt ,    R,  R(dest) = (SEXT(src1,32)< SEXT(src2,32)) ? 1:0 );
//store
...
//mul-longlong  SEXT(BITS(src1>>src2,31,0),32)
...
//shift有问题
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw ,   R, R(dest) = SEXT(BITS(SEXT(src1,32) >> BITS(src2,4,0),31,0),32) );
...
//hello-str
  //INSTPAT("??????? ????? ????? 110 ????? 00100 11", ori ,    R, R(dest) = SEXT(BITS(src1|src2,31,0),32)  , printf("cuRent pc is %lx ",s->pc),printf("R(10) is:%lx\n",R(10)));
...
//coremark  
  INSTPAT("0000001 ????? ????? 101 ????? 01110 11", divuw ,  R, R(dest) = SEXT(BITS(BITS(src1,31,0) /(unsigned int)BITS(src2,31,0),31,0),32) , printf("cuRent pc is %lx ",s->pc),printf("R(10) is:%lx\n",R(10)));
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb ,    I, R(dest) = SEXT(BITS(Mr(src1 + src2, 1),7,0),8 ) , printf("cuRent pc is %lx ",s->pc),printf("R(10) is:%lx\n",R(10)));

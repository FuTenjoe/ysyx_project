...
INSTPAT_START();//INSTPAT(模式字符串, 指令名称, 指令类型, 指令执行操作);
  ...
  //INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw,   I, R(dest) = cpu.sr[BITS(src2,3,0)], cpu.sr[BITS(src2,3,0)] = src1);
   INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs,   I, R(dest) = cpu.sr[src2], cpu.sr[src2] = cpu.sr[src2] | src1,printf("cpu.sr[src2] = %ld",cpu.sr[src2]));
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw,   I, R(dest) = cpu.sr[src2], cpu.sr[src2] = src1,printf("cpu.sr[src2] = %ld",cpu.sr[src2]));
